export const zhMessages = {
  ...
  customFaceEditor: {
    ...
    timeline: {
      label: '表情时间轴', play: '预览全部', pause: '暂停', selectedPreview: '预览选中', totalFrames: '总帧数 {{count}}', totalDuration: '总时长 {{duration}} ms', selected: '已选 {{count}} 帧', current: '当前帧 {{current}} / {{total}}', insert: '插入位置', selectFrame: '勾选第 {{index}} 帧', framePreview: '第 {{index}} 帧预览', frame: '帧 {{index}}', deleteFrame: '删除帧 {{index}}', keepOne: '至少保留一帧', addFrame: '新增帧', addFrameHint: '复制当前帧并追加到末尾', addFrameOptions: '选择新增帧方式', copyFrame: '复制当前帧', blankFrame: '创建空白帧'
    },
    rulers: {
    ...
    canvasOverlay: { pendingImport: '待确认导入对象', pendingImportPreview: '待确认导入 {{width}} × {{height}}', magnifierArea: '放大区域', currentPixel: '当前像素', originalSelection: '原始选区', selection: '当前选区', selectionPivot: '旋转轴', eraserArea: '擦除范围', toolPreview: '工具预览', penPreview: '钢笔路径预览', pixelGrid: '像素网格' },
    ...
    svgImport: { title: '导入 SVG', filterName: 'SVG 图像', mergeLabel: '叠加到当前帧', replaceLabel: '覆盖当前帧', description: '使用 SVG 原生静态渲染转换为单色像素。推荐透明背景和任意不透明前景；不透明底色也会被点亮。', read: '读取中…', selectFile: '选择 SVG 文件', importImage: '从图片生成', metadata: '原始尺寸 {{width}} × {{height}} · {{elements}} 个元素 · 目标分辨率 {{targetWidth}} × {{targetHeight}}', noFile: '请选择 SVG 文件', updating: '正在更新预览…', mode: '导入方式', merge: '叠加', replace: '覆盖', x: '水平位置 X', y: '垂直偏移', offsetXLabel: '水平偏移', offsetYLabel: '垂直偏移', scaleLabel: '缩放', rotationLabel: '旋转', thresholdLabel: '像素覆盖阈值', recognitionMode: '像素识别方式', alphaRecognition: '按透明度', alphaRecognitionLabel: '按透明度识别像素', alphaRecognitionHint: '适合透明背景素材；只根据像素透明度判断亮灭。', brightnessRecognition: '按亮度', brightnessRecognitionLabel: '按亮度识别像素', brightnessRecognitionHint: '适合白底黑图；深色像素更容易转换为亮点，透明像素仍保持熄灭。', decrease: '减少{{field}}', increase: '增加{{field}}', center: '居中', topLeft: '左上角', reset: '重置', offsetX: '水平偏移：{{value}}', offsetY: '垂直偏移：{{value}}', scale: '缩放：{{value}}', rotation: '旋转：{{value}}°', threshold: '像素覆盖阈值：{{value}}', thresholdHint: '低值保留更多细线，高值会收紧边缘。', activePixels: '高亮像素：{{count}}', clipped: '已裁切屏幕外内容', cancel: '取消', apply: '应用到当前帧', preview: 'SVG 预览 {{width}} × {{height}}' },
    imageVectorizer: {
      title: '图片转 SVG',
      selectImage: '选择图片',
      selectedFile: '已选图片：{{name}}',
