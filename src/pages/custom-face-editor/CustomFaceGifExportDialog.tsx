...
type Props = {
  ...
  frameCount: number;
  onCancel: () => void;
  onConfirm: (options: { scale: number; invert: boolean; transparentBackground: boolean; frameIndices: number[] }) => void;
};

const SCALES = [1, 2, 3, 4, 5, 6, 7, 8];

export function CustomFaceGifExportDialog({ open, width, height, frameCount, onCancel, onConfirm }: Props) {
  const t = useI18n();
  ...
  const handleConfirm = () => {
    const frameIndices = frameSelection === 'all'
      ? []
  ...
  return <Dialog open={open} onOpenChange={(nextOpen) => !nextOpen && onCancel()}>
    <DialogContent>
      ...
      <div className="space-y-3">
        ...
        <label className="flex items-center gap-2">
          <Checkbox checked={transparentBackground} onCheckedChange={(checked) => setTransparentBackground(checked === true)} />
          <span className="text-sm">{t('customFaceEditor.faceExport.dialog.transparentBackground')}</span>
        </label>
      </div>
      {frameCount > 1 && (
        <div className="space-y-2">
          <label className="block text-sm font-medium">{t('customFaceEditor.faceExport.dialog.frames')}</label>
          <div className="space-y-2">
            <label className="flex items-center gap-2">
              <input type="radio" checked={frameSelection === 'all'} onChange={() => setFrameSelection('all')} />
              <span className="text-sm">{t('customFaceEditor.faceExport.dialog.allFrames')}</span>
            </label>
            <label className="flex items-center gap-2">
              <input type="radio" checked={frameSelection === 'custom'} onChange={() => setFrameSelection('custom')} />
              <span className="text-sm">{t('customFaceEditor.faceExport.dialog.customFrames')}</span>
            </label>
            {frameSelection === 'custom' && (
              <div className="ml-6 space-y-2">
                <div className="flex gap-2">
                  <button type="button" className="border border-border px-2 py-1 text-xs" onClick={handleSelectAll}>{t('customFaceEditor.faceExport.dialog.selectAll')}</button>
                  <button type="button" className="border border-border px-2 py-1 text-xs" onClick={handleDeselectAll}>{t('customFaceEditor.faceExport.dialog.deselectAll')}</button>
                </div>
                ...
                <p className="text-xs text-muted-foreground">
                  {t('customFaceEditor.faceExport.dialog.selectedCount', { count: selectedFrames.size })}
                </p>
              </div>
            )}
          </div>
      ...
      <DialogFooter>
        ...
        <button type="button" className="border border-primary bg-primary px-3 py-2 text-primary-foreground" onClick={handleConfirm}>{t('customFaceEditor.faceExport.dialog.confirm')}</button>
      </DialogFooter>
    </DialogContent>
  </Dialog>;
}
