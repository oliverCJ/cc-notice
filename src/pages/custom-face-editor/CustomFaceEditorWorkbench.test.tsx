...
const exportCustomFaceGifMock = vi.hoisted(() =>
  vi.fn().mockResolvedValue({ frameDelaysMs: [210], totalDurationMs: 210 })
);
...
const windowApiMock = vi.hoisted(() => ({
  closeRequestedHandler: null as
    null | ((event: { preventDefault: () => void }) => void | Promise<void>),
  closeRequestedHandlers: [] as Array<
...
test('rotates a selected region from the workbench controls', async () => {
  ...
  await waitFor(() =>
    expect(screen.getByRole('button', { name: '设置旋转轴' })).toBeInTheDocument()
  );
...
test('shows a prominent banner while the image import window is open', async () => {
  ...
  await waitFor(() => {
    imagePixelizerStateListener = listenMock.mock.calls.find(
      (call) => call[0] === 'cc-notice://custom-face-image-pixelizer-state-changed'
    )?.[1] as ((event: { payload: boolean }) => void) | undefined;
...
test('allows closing the editor after the image import window reports closed', async () => {
  ...
  expect(
    screen.queryByText('图片导入窗口已打开，请先关闭它再退出编辑器。')
...
test('blocks editor close while the image vectorizer window is open', async () => {
  ...
  await waitFor(() => {
    vectorizerStateListener = listenMock.mock.calls.find(
      (call) => call[0] === 'cc-notice://custom-face-image-vectorizer-state-changed'
    )?.[1] as ((event: { payload: boolean }) => void) | undefined;
  ...
  await waitFor(() =>
    expect(
      screen.getAllByText('图片转 SVG 窗口已打开，请先关闭它再退出编辑器。').length
    ).toBeGreaterThan(0)
  );
});
...
test('opens the svg import dialog after receiving a vectorized svg path', async () => {
  readCustomFaceSvgMock.mockResolvedValue(
  ...
  await waitFor(() => {
    svgPathListener = listenMock.mock.calls.find(
      (call) => call[0] === CUSTOM_FACE_OPEN_SVG_PATH_EVENT
    )?.[1] as ((event: { payload: string }) => void) | undefined;
...
test('lets a vectorized svg become a movable pending canvas object after applying it', async () => {
  ...
  expect(pendingPreview).toHaveStyle({
    width: '1px',
    height: '1px',
...
test('does not let a stale image import close handler block editor close', async () => {
  ...
  const handlerRegisteredWhileOpen =
    windowApiMock.closeRequestedHandlers[windowApiMock.closeRequestedHandlers.length - 1];
  await act(async () => {
    imagePixelizerStateListener?.({ payload: false });
  });
...
test('loads imported image pixels into the pending canvas workflow', async () => {
  ...
  let imageImportListener:
    | ((event: {
        payload: { packedPixels: number[]; sourceWidth: number; sourceHeight: number };
      }) => void)
    | undefined;
  await waitFor(() => {
    imageImportListener = listenMock.mock.calls.find(
      (call) => call[0] === 'cc-notice://custom-face-image-import-ready'
    )?.[1] as
      | ((event: {
          payload: { packedPixels: number[]; sourceWidth: number; sourceHeight: number };
        }) => void)
...
test('blocks a mismatched face profile before opening import confirmation', async () => {
  previewCustomFaceItemImportMock.mockResolvedValue({
    face: {
      ...
      name: 'Imported',
      color: { red: 1, green: 2, blue: 3 },
      frames: [{ durationMs: 200, packedPixels: Array(1024).fill(0) }],
    },
...
test('exports the current unsaved face snapshot without saving', async () => {
  ...
  expect(exportCustomFaceGifMock).toHaveBeenCalledWith(
    expect.objectContaining({
      scale: 2,
      face: expect.objectContaining({ frames: [expect.objectContaining({ durationMs: 205 })] }),
    })
  );
  expect(saveCustomFaceGroupMock).not.toHaveBeenCalled();
...
test('renames the group in the editor and stays clean after saving', async () => {
  ...
  fireEvent.change(screen.getByRole('textbox', { name: '组名称' }), {
    target: { value: '新的组名' },
  });
...
test('offers an explicit overwrite when the saved library hash conflicts', async () => {
  ...
  expect(saveCustomFaceGroupMock.mock.calls[1][0]).toEqual(
    expect.objectContaining({ expectedLibraryHash: 'disk-hash' })
  );
  expect(screen.getByText('已保存')).toBeInTheDocument();
});
...
test('reloads the disk group after a save conflict', async () => {
  ...
  saveCustomFaceGroupMock.mockRejectedValue(
    new Error('custom face group conflicts with current hash disk-hash')
  );
  ...
  await waitFor(() =>
    expect(screen.getByRole('textbox', { name: '组名称' })).toHaveValue('磁盘版本')
  );
  expect(onSaved).toHaveBeenCalledWith({
    group: diskGroup,
    libraryHash: 'disk-hash',
    changed: false,
  });
  expect(screen.getByText('已保存')).toBeInTheDocument();
});
...
test('prevents a dirty native window close and supports cancel or discard', async () => {
  ...
  await act(async () => {
    await windowApiMock.closeRequestedHandler?.({ preventDefault });
  });
