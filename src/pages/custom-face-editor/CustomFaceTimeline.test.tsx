...
test('shows frame duration badges and total duration', () => {
  render(
    <CustomFaceTimeline
      frames={[
        { durationMs: 200, packedPixels: [0] },
        { durationMs: 400, packedPixels: [1] },
      ]}
      selectedIndex={0}
      onSelect={() => undefined}
      onAdd={() => undefined}
...
test('uses the main add button to copy the current frame and exposes a blank-frame option', async () => {
  render(
    <CustomFaceTimeline
      frames={[{ durationMs: 200, packedPixels: [0] }]}
      selectedIndex={0}
      onSelect={() => undefined}
      onAdd={onAdd}
      onDelete={() => undefined}
  ...
  await waitFor(() =>
    expect(screen.getByRole('menuitem', { name: '复制当前帧' })).toBeInTheDocument()
  );
...
test('renders actual frame pixels in each timeline item', () => {
  render(
    <CustomFaceTimeline
      frames={[{ durationMs: 200, packedPixels: [1, ...Array(511).fill(0)] }]}
      selectedIndex={0}
      onSelect={() => undefined}
      onAdd={() => undefined}
      onDelete={() => undefined}
