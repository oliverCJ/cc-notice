...
test('moves a magnifier viewport origin and clamps it to the canvas', () => {
  ...
  expect(moveMagnifierViewportOrigin(viewport, [3, -2], 128, 32)).toEqual({ ...viewport, originX: 13, originY: 6 });
  expect(moveMagnifierViewportOrigin(viewport, [-100, -100], 128, 32)).toEqual({ ...viewport, originX: 0, originY: 0 });
  expect(moveMagnifierViewportOrigin(viewport, [1000, 1000], 128, 32)).toEqual({ ...viewport, originX: 112, originY: 20 });
  expect(moveMagnifierViewportOrigin({ originX: 0, originY: 0, width: 8, height: 4 }, [3, 3], 8, 4)).toEqual({ originX: 0, originY: 0, width: 8, height: 4 });
});
...
test('maps magnifier client coordinates back to full canvas pixels', () => {
  ...
  expect(logicalPointFromViewportClientPoint(rect, 3.5, 6.25, 320, 240, viewport)).toEqual([304, 228]);
  expect(logicalPointFromViewportClientPoint(rect, 163.49, 126.24, 320, 240, viewport)).toEqual([319, 239]);
  expect(logicalPointFromViewportClientPoint(rect, 999, -1, 320, 240, viewport)).toEqual([319, 228]);
});
