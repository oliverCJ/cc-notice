...
vi.mock('@/api/tauriApi', async () => {
  ...
  return {
    ...actual,
    openCustomFaceImageVectorizer: openVectorizerMock,
    readCustomFaceSvg: readSvgMock,
  };
});
...
test('uses brightness recognition by default for vectorizer supplied svg paths', async () => {
  ...
  await waitFor(() => expect(readSvgMock).toHaveBeenCalledWith('/tmp/vectorized.svg'));
  await waitFor(() =>
    expect(rasterizeMock).toHaveBeenLastCalledWith(
      expect.anything(),
      { width: 8, height: 8 },
...
test('opens the image vectorizer from the svg import dialog', async () => {
  render(
    <CustomFaceSvgImportDialog open width={8} height={8} onCancel={vi.fn()} onApply={vi.fn()} />
  );

  expect(
    screen.getByRole('button', { name: '选择 SVG 文件' }).parentElement
  ).toHaveClass('flex', 'flex-nowrap', 'items-center', 'gap-2');
...
test('selects only svg files, previews target resolution and applies packed pixels once', async () => {
  ...
  readSvgMock.mockResolvedValue(
    '<svg viewBox="0 0 8 8"><rect x="1" y="1" width="3" height="2" /></svg>'
  );
  const onApply = vi.fn();

  render(
    <CustomFaceSvgImportDialog open width={8} height={8} onCancel={vi.fn()} onApply={onApply} />
  );
  ...
  fireEvent.change(screen.getByRole('spinbutton', { name: '水平位置 X' }), {
    target: { value: '3' },
  });
...
test('switches pixel recognition mode and passes it to rasterization', async () => {
  ...
  expect(screen.getByRole('radio', { name: '按亮度识别像素' })).toBeChecked();
  expect(
    screen.getByText('适合白底黑图；深色像素更容易转换为亮点，透明像素仍保持熄灭。')
  ).toBeInTheDocument();
  await waitFor(() =>
    expect(rasterizeMock).toHaveBeenLastCalledWith(
      expect.anything(),
...
test('supports precise range adjustments with step buttons and boundaries', async () => {
  ...
  render(
    <CustomFaceSvgImportDialog open width={8} height={6} onCancel={vi.fn()} onApply={vi.fn()} />
  );
  fireEvent.click(screen.getByRole('button', { name: '选择 SVG 文件' }));
  await waitFor(() => expect(screen.getByText(/高亮像素：1/)).toBeInTheDocument());
  ...
  fireEvent.change(screen.getByRole('slider', { name: '像素覆盖阈值' }), {
    target: { value: '100' },
  });
...
test('keeps the completed preview visible while parameter rendering is pending', async () => {
  openMock.mockResolvedValue('/tmp/face.svg');
  readSvgMock.mockResolvedValue(
    '<svg viewBox="0 0 8 8"><rect x="1" y="1" width="2" height="2" /></svg>'
  );
...
test('allows switching SVG import from merge to replace mode', async () => {
  ...
  fireEvent.click(screen.getByRole('button', { name: '选择 SVG 文件' }));
  await waitFor(() =>
    expect(screen.getByRole('radio', { name: '覆盖当前帧' })).toBeInTheDocument()
  );
  await waitFor(() => expect(screen.getByRole('button', { name: '应用到当前帧' })).toBeEnabled());
