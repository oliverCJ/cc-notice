...
const group: CustomFaceGroupSummary = {
  groupId: 'g',
  name: '测试组',
...
test('submits target profile and editable name in resize dialog', () => {
  ...
  render(
    <ResizeGroupDialog group={group} open busy={false} onCancel={vi.fn()} onConfirm={onConfirm} />
  );
  fireEvent.change(screen.getByRole('textbox', { name: '新组名称' }), {
    target: { value: '目标组' },
  });
...
test('requires explicit confirmation before deleting a group', () => {
  const onConfirm = vi.fn();
  render(
    <DeleteGroupDialog group={group} open busy={false} onCancel={vi.fn()} onConfirm={onConfirm} />
  );
...
test('submits the edited group name', () => {
  const onConfirm = vi.fn();
  render(
    <RenameGroupDialog group={group} open busy={false} onCancel={vi.fn()} onConfirm={onConfirm} />
  );
