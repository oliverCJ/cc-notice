...
test('confirms a renamed single face import without replacement options', () => {
  ...
  render(<CustomFaceItemImportDialog open preview={preview} maxFacesReached={false} duplicate onCancel={vi.fn()} onConfirm={onConfirm} />);

  expect(screen.getByText('当前组中已有内容相同的表情，仍可导入副本。')).toBeInTheDocument();
  fireEvent.change(screen.getByRole('textbox', { name: '导入后的表情名称' }), { target: { value: 'Ready copy' } });
...
test('blocks confirmation when the target group is full', () => {
  render(<CustomFaceItemImportDialog open preview={preview} maxFacesReached duplicate={false} onCancel={vi.fn()} onConfirm={vi.fn()} />);
