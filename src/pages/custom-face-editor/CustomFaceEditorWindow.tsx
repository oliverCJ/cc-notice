...
export function CustomFaceEditorWindow() {
  ...
  useEffect(() => { void getCustomFaceGroups().then(async (items) => { setGroups(items); const loaded = await Promise.all(items.map(async (item) => [item.groupId, await getCustomFaceGroup(item.groupId)] as const)); setPreviews(Object.fromEntries(loaded)); }).finally(() => setLoading(false)); }, []);
  useEffect(() => {
    if (current) return;
    ...
    let currentWindow;
    try { currentWindow = getCurrentWindow(); } catch (error) { console.warn('failed to access custom face editor window', error); return; }
    void currentWindow.onCloseRequested((event) => {
      if (closingRef.current) return;
      event.preventDefault();
      closingRef.current = true;
      void closeCustomFaceEditor().catch((error) => { closingRef.current = false; console.warn('failed to close custom face editor library', error); });
  ...
  if (!current) {
    return <CustomFaceGroupLibrary groups={groups} previews={previews} onOpen={async (id) => setCurrent(await getCustomFaceGroup(id))} onCreate={async (profileId) => {
      ...
      const saved = await saveCustomFaceGroup({ group: { ...converted, name }, expectedLibraryHash: null }); addSaved(saved); setPreviews((items) => ({ ...items, [saved.group.groupId]: saved.group })); setCurrent(saved.group);
    }} onRename={async (id, name) => {
      const item = groups.find((group) => group.groupId === id); if (!item) return;
      const saved = await saveCustomFaceGroup({ group: { ...await getCustomFaceGroup(id), name }, expectedLibraryHash: item.libraryHash });
      setGroups((items) => items.map((group) => group.groupId === id ? summary(saved) : group)); setPreviews((items) => ({ ...items, [id]: saved.group }));
    }} onDelete={async (id) => {
      const item = groups.find((group) => group.groupId === id); if (!item) return;
      await deleteCustomFaceGroup(id, item.libraryHash); setGroups((items) => items.filter((group) => group.groupId !== id)); setPreviews((items) => { const next = { ...items }; delete next[id]; return next; });
    }} onImport={async () => {
      const path = await open({ multiple: false, directory: false, filters: [{ name: 'CC Face Group', extensions: ['ccface'] }] });
      ...
      const path = await save({ defaultPath: `${groups.find((item) => item.groupId === id)?.name ?? 'custom-face'}.ccface`, filters: [{ name: 'CC Face Group', extensions: ['ccface'] }] });
