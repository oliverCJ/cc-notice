...
export function editorProfileFromId(id: string): EditorProfile | null {
  ...
  if (preset) return { id: preset.id, width: preset.width, height: preset.height, maxFrames: preset.maxFrames, framebufferBytes: preset.framebufferBytes };
  ...
  return { id, width, height, maxFrames: maxFramesForResolution(width, height), framebufferBytes: width * Math.ceil(height / 8) };
}

export function customProfileId(width: number, height: number) { return `custom-${Math.trunc(width)}x${Math.trunc(height)}-v1`; }
