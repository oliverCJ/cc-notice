...
test('app tests do not depend on firmware source outside the app checkout', () => {
  ...
  expect(firmwareSourceTest).not.toContain("resolve(appRoot, '..'");
  expect(firmwareSourceTest).not.toContain('readWorkspaceFile');
  expect(
    readFileSync(
      join(process.cwd(), 'src/domain/display/generated/displayFaceContract.generated.ts'),
      'utf8'
    )
  ).toContain('generated by firmware/tools/generate-display-face-contract.mjs');
  expect(
    readFileSync(
      join(process.cwd(), 'src/domain/display/generated/displayFaceGolden.generated.ts'),
      'utf8'
    )
  ).toContain('generated by firmware/tools/export-display-face-app-fixtures.mjs');
  expect(JSON.stringify(JSON.parse(readFileSync(join(process.cwd(), 'package.json'), 'utf8'))))
    .not.toContain('../firmware');
});
